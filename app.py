...
st.set_page_config(page_title="数字融合实验室", page_icon="🌟", layout="centered")
...
<style>
    ...
    .log-area { background-color: #ffffff; border-radius: 15px; padding: 15px; margin-top: 10px; border: 4px dashed #ffcccb; max-height: 480px; overflow-y: auto; }
    ...
    .carry-row { display: flex; align-items: center; justify-content: center; gap: 15px; flex-wrap: nowrap; }
    @media (max-width: 600px) {
        .carry-row { flex-wrap: wrap; }
        .carry-10 { max-width: 100% !important; }
    }
    .carry-10 { font-size: 1.8rem; line-height: 1.3; max-width: 50%; text-align: center;}
    .carry-arrow { font-size: 2rem; font-weight: bold; }
    .carry-1 { font-size: 2rem; text-align: center;}
...
def html_for_add(amount):
    if amount <= 20:
        icons = " ".join(["🍏"] * amount)
        return f"<div class='log-card add-card'>➕ 获得能量球：<br><span style='font-size: 2.5rem;'>{icons}</span></div>"
    else:
        return f"<div class='log-card add-card'>➕ 获得大量能量球：<span style='font-size: 2rem;'>{amount}</span> 个 🍏</div>"
...
def html_for_carry(from_level, to_level):
    ...
    <div class='log-card carry-card' style='background-color: {bg_color}; border: 3px solid {border_color};'>
        <div class='carry-row'>
            ...
            <div class='carry-1'>
                <span style='font-size: 4rem;'>{to_emoji}</span><br>
...
def process_energy(amount):
    ...
    if carry_happened:
        st.session_state.message = ("success", "✨ 发生了一连串进位大融合！快向下看图形战报！")
    else:
...
st.button("🧹 重置并清空实验室", use_container_width=True, on_click=on_reset)

st.write("---")

st.subheader("📜 图形化连环爆破战报")
if not st.session_state.logs:
    st.markdown("<div class='log-area'><p style='text-align:center; color:gray; font-size:1.5rem;'>这里空空的，快去上面点击发送能量球吧！👆</p></div>", unsafe_allow_html=True)
else:
